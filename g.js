document.addEventListener('DOMContentLoaded', () =>  {
    ...
    function generate() {
        ...
        if (squares[randomNumber].innerHTML == 0) {
            ...
            generate()  // Try again if the spot is not empty
        }
    }
    ...
    // Combine numbers in rows if they're the same
    function combineRow() {
        for (let i = 0; i < 15; i++) {
    ...
    // Combine numbers in columns if they're the same
    ...
    // Move numbers and combine them when keys are pressed
    function control(e) {
        if(e.keyCode === 37) {
            moveLeft()
            combineRow()
    ...
    // Check if the game is over means no zeros are left.
    ...
    // Change colors based on the number
    function addColours() {
